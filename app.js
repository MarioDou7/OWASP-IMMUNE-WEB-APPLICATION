...
const limiter = rateLimit({
    ...
    message: 'Too many Login Attempts from this IP, please try again after an 1 Minute',
    legacyHeaders: false, // Disable the `X-RateLimit-*` headers
    blockDuration: 60, //block duration with seconds
})
...
app.post("/adminSearch", function(req, res) {
    var usertoSearch = req.body.Suser;
    var respones = db.search_user(usertoSearch);
    if (respones) {
        return res.render("admin", { message: respones.message })
    }
});

app.post("/adminDelete", function(req, res) {
    var usertoDelete = req.body.Duser;
    var respones = db.delete_user(usertoDelete);
    if (respones) {
        return res.render("admin", { message: respones.message })
    }
});
